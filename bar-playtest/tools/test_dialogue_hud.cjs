...
(async()=>{let browser;try{
 ...
 for(const [width,height] of [[1742,981],[1920,1080],[820,650],[600,800],[1280,720]]){
  ...
  const measure=await p.evaluate(()=>{const shell=document.querySelector('.app-shell').getBoundingClientRect(),hud=document.querySelector('.currency-hud').getBoundingClientRect(),status=document.querySelector('.statusbar').getBoundingClientRect(),left=document.querySelector('.top-controls').getBoundingClientRect(),dot=document.querySelector('.seat-indicators span').getBoundingClientRect(),box=document.querySelector('.dialogue-box').getBoundingClientRect();return {fits:hud.right<=shell.right&&hud.top>=shell.top,statusFits:status.right<hud.left,leftFits:left.right<status.left,ratio:box.width/shell.width,dot:dot.width/shell.width};});
  assert(measure.fits&&measure.statusFits&&measure.leftFits);assert(Math.abs(measure.ratio-538/1280)<.001);assert(Math.abs(measure.dot-14/1280)<.001);
 }
