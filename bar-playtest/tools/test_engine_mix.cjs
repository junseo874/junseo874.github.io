...
const D=ctx.window.LUNA_DATA,C=require(dir+'/core'),R=require(dir+'/remix'),M=require(dir+'/minigames');
let count=0;function test(name,fn){fn();count++;console.log('PASS',name);}
function game(kind,variant='original'){const g=new C.Game(D);R.attach(g);M.attach(g);g.startMinigame(kind,variant,'challenge');return g;}
function tick(g,t){for(let remain=t;remain>1e-9;){const d=Math.min(.01,remain);g.tick(d);remain-=d;}}
function start(g){g.gimmickInput(g.gimmick.type==='stir'?'KeyW':'MouseLeft');}
test('no autonomous character animation, score, or prep timer',()=>{for(const kind of ['shake','stir']){const g=game(kind),s=g.gimmick,before=JSON.stringify(s);tick(g,3);assert.equal(JSON.stringify(s),before);assert.equal(g.craft.elapsed,0);start(g);if(kind==='shake'){tick(g,7);assert.equal(s.attempts,0);assert.equal(s.motionFrame,0);assert.equal(s.motionAge,null);}else{tick(g,2.01);assert.equal(s.failures,1);assert.equal(s.spoonAngle,0);assert.equal(s.swirlSpeed,0);}}});
...
test('web stir limit is 1.5 seconds in both versions, with pause and reset',()=>{
 for(const variant of ['original','gpt']){
  const g=game('stir',variant),s=g.gimmick;assert.equal(g.c('stir_circle_limit_sec'),1.5);start(g);
  tick(g,1.49);assert.equal(s.failures,0);
  g.overlay='settings';tick(g,2);assert.equal(s.failures,0);g.overlay=null;
  tick(g,.02);assert.equal(s.failures,1);assert(s.circleTime<.02);
  for(const key of ['KeyD','KeyS','KeyA','KeyW'])g.gimmickInput(key);
  assert.equal(s.success,1);assert.equal(s.circleTime,0);
  tick(g,1.49);assert.equal(s.failures,1);tick(g,.02);assert.equal(s.failures,2);
 }
});
