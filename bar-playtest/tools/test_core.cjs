...
test('opening ring, shake rhythm and clockwise stirring accept real inputs',()=>{
 ...
 const shaker=game(99,'practice');shaker.selectCocktail('gin_fizz');shaker.debugFill();shaker.startCraft();shaker.craft.index=shaker.craft.queue.findIndex(q=>q.type==='shake');shaker.nextGimmick();shaker.gimmickInput('Space');for(let i=0;i<6000&&!shaker.gimmick.completed;i++){shaker.tick(.005);if(C.MIX.nearest(shaker.gimmick)&&!shaker.gimmick.feedbackLeft)shaker.gimmickInput('Space');}assert.equal(shaker.gimmick.success,20);assert.equal(shaker.gimmick.attempts,20);
