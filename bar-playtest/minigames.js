...
function createUI({g,ui,L,esc,button,itemArt,tabsHTML,mini}){
 ...
 function lobbyHTML(){return `<div class="start-screen minigame-lobby">${tabsHTML()}<section class="minigame-options"><div><span>${L('플레이 규칙','Rules')}</span><div class="row">${[['gpt','GPT 개선 규칙','GPT Remix'],['original','기존 규칙','Original']].map(([id,ko,en])=>button(L(ko,en),'miniRules',`data-id="${id}" aria-pressed="${variant()===id}"`,variant()===id?'selected':'')).join('')}</div></div><div><span>${L('개선 규칙 난이도','Remix difficulty')}</span><div class="row">${[['cozy','편안하게','Cozy'],['standard','보통','Standard'],['challenge','도전','Challenge']].map(([id,ko,en])=>button(L(ko,en),'miniDifficulty',`data-id="${id}" aria-pressed="${difficulty()===id}" ${variant()==='original'?'disabled':''}`,difficulty()===id?'selected':'')).join('')}</div></div></section><div class="minigame-cards">${KINDS.map((kind,i)=>`<button class="minigame-card" data-act="miniStart" data-id="${kind}"><span class="minigame-number">0${i+1}</span><div class="minigame-art">${itemArt(art(kind))}</div><h2>${name(kind)}</h2><p>${description(kind)}</p><strong>${goal(kind)}</strong><small>${control(kind)}</small><span class="minigame-enter">${L('바로 시작 →','Play now →')}</span></button>`).join('')}</div><footer>${button(L('조작 안내','Controls'),'help','','subtle')}</footer></div>`;}
