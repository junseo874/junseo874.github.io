...
function attach(g){
 ...
 g.endGimmick=function(){if(this.minigame&&(!this.gimmick||!['pour','fill_up'].includes(this.gimmick.type)&&!this.gimmick.completed))return false;return end();};
...
function createUI({g,ui,L,esc,button,itemArt,tabsHTML,mini}){
 ...
 function control(kind){return kind==='stir'?L('W 시작 · D → S → A → W','Start W · D → S → A → W'):['pour','fill_up'].includes(kind)?L('Space 누르고 놓기','Hold & release Space'):L('Space로 시작하고 타이밍에 다시 누르기','Space to start, then press on target');}
